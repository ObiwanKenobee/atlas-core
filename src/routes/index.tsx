...
import { AGENTS, EVENTS, TIMESERIES, fmtTime } from "@/lib/atlas-data";
...
const stats = [
  { label: "Agents Online", value: "4 / 4", icon: Cpu, tone: "text-signal" },
  { label: "Events / 24h", value: "1,284", icon: Activity, tone: "text-foreground" },
  { label: "Risk Index", value: "0.46", icon: TrendingUp, tone: "text-warn" },
  { label: "Active Alerts", value: "2", icon: AlertTriangle, tone: "text-danger" },
];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-1">
        <span className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">Mission Control</span>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Panel title="Agent Stack" meta="RUNTIME">
      ...
      {/* Telemetry chart + Event stream */}
      <div className="grid gap-4 lg:grid-cols-3">
        <Panel title="Signal Telemetry" meta="T-30 → T0" className="lg:col-span-2">
          <div className="h-56 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={TIMESERIES} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="ecological" stroke="oklch(0.82 0.16 195)" fill="url(#g1)" strokeWidth={1.5} />
                <Area type="monotone" dataKey="economic" stroke="oklch(0.78 0.17 145)" fill="url(#g2)" strokeWidth={1.5} />
                <Area type="monotone" dataKey="risk" stroke="oklch(0.68 0.22 25)" fill="url(#g3)" strokeWidth={1.5} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          <div className="mt-2 flex gap-4 text-[10px] uppercase tracking-widest text-muted-foreground">
            <span><span className="mr-1 inline-block h-2 w-2 bg-[oklch(0.82_0.16_195)]" />ecological</span>
            <span><span className="mr-1 inline-block h-2 w-2 bg-[oklch(0.78_0.17_145)]" />economic</span>
            <span><span className="mr-1 inline-block h-2 w-2 bg-[oklch(0.68_0.22_25)]" />risk</span>
          </div>
        </Panel>
        ...
        <Panel title="Event Stream" meta="LIVE">
          <ul className="space-y-1.5 font-mono text-[11px] leading-tight">
            {EVENTS.map((e) => (
              <li key={e.id} className="border-l-2 border-border/60 pl-2"
